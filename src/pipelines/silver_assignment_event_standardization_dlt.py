...
@dlt.table(
    name="assignment_event_standardized_dlt",
    comment=(
        "Standardized assignment change events from Bronze. "
        "This table normalizes keys/codes, parses timestamps, casts sequence, "
        "and preserves source metadata."
    ),
)
def assignment_event_standardized_dlt():
    bronze_df = spark.read.table(BRONZE_ASSIGNMENT_TABLE)
